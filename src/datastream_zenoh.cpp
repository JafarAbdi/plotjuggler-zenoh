...
DataStreamZenoh::DataStreamZenoh()
{
  ...
  for (int i = 0; i < 150; i++)
  {
    ...
    DataStreamZenoh::Parameters param;
...
bool DataStreamZenoh::start(QStringList*)
{
  _running = true;
  pushSingleCycle();
...
bool DataStreamZenoh::isRunning() const
{
  return _running;
}
...
bool DataStreamZenoh::xmlSaveState(QDomDocument& doc, QDomElement& parent_element) const
{
  return true;
}

bool DataStreamZenoh::xmlLoadState(const QDomElement& parent_element)
{
  return true;
}

void DataStreamZenoh::pushSingleCycle()
{
  static int count = 0;
  ...
  for (auto& it : _parameters)
  {
    ...
    const DataStreamZenoh::Parameters& param = it.second;
